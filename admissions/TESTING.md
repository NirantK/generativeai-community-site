...
## Student decisions and agent-origin appeals

Policy v4 defaults clear student applications to decline unless exceptional original work is supported, with approved company affiliations preserved as an exception. New tests cover the exception, malformed student assessments, uncertain classifications, automatic rejection without an invitation, immutable submission provenance, form/legacy ineligibility, required evidence, unsafe links, CSRF, expired/revoked tokens, appeal idempotency, pending-appeal conflicts, new evidence after another rejection, and full manual resolution history. Browser coverage includes agent appeal submission, hidden form-applicant appeals, pending appeal indicators, and escaped reference claims in the admin panel. No real applicant was rejected and no live appeal was fabricated during testing.

## Past Chats archive

47 backend/page-gate tests, 4 Python importer tests, and 225 full-site browser checks passed locally for this change. Coverage includes current Agent approval, browser/token reads, bearer exclusion from administration, revocation, publication filtering, FTS/date/group search, cursor validation, context isolation, CSRF, moderation audit records, HTML-as-text rendering, mobile accessibility, import idempotency/edits, metadata exclusion, and persistent deletion/hiding. Synthetic fixtures only: no real group history was exported or published during these tests. Source-group selection and actual history coverage remain separate operational checks.
