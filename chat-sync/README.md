...
4. With scheduling still paused, trigger the authenticated workflow with `{"force":true}` for a controlled sync. After verification, set `SYNC_ENABLED=true` through a GitHub configuration change. Verify it reports success, D1 index integrity, preserved legacy counts/publication status, and accurate coverage. Trigger again to confirm idempotency, then confirm restoration after container stop.
