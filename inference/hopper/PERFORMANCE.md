...
Fresh Modal A10 function invocations took **85.8 s upstream** (78.6 s model initialization and kernel warm-up) and **22.8 s Q8_0** (15.3 s model load and first score), measured from `remote()` call to response in two new ephemeral apps. The difference includes scheduling, container setup, and volume mount under the then-current image/cache state. It does **not** include CLI startup or a public HTTP endpoint. With a 30-second idle allowance, ten decisions per activation, and each runtime's measured warm mean, this implies a **scenario estimate** of $0.00503 per completed decision upstream versus $0.00235 Q8_0. The candidate's cold-start economics are attractive at sparse traffic, but its failed fidelity and p95 gates prevent promotion. These are single-run first-invocation measurements, not a cold-start distribution.
